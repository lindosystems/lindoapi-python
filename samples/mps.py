...
lindo.pyLSloadLicenseString(os.getenv('LINDOAPI_HOME')+'/license/lndapi140.lic',LicenseKey)
...
mpifile = os.getenv('LINDOAPI_HOME') + '/samples/data/testgop.mpi'
errorcode = lindo.pyLSreadMPIFile(pModel,mpifile)
lindo.geterrormessage(pEnv,errorcode)

errorcode = lindo.pyLSgetInfo(pModel,lindo.LS_IINFO_NUM_VARS,numVars)
lindo.geterrormessage(pEnv,errorcode)
errorcode = lindo.pyLSgetInfo(pModel,lindo.LS_IINFO_NUM_CONS,numCons)
lindo.geterrormessage(pEnv,errorcode)
errorcode = lindo.pyLSgetInfo(pModel,lindo.LS_IINFO_NUM_NLP_VARS,numNlpVars)
lindo.geterrormessage(pEnv,errorcode)
errorcode = lindo.pyLSgetInfo(pModel,lindo.LS_IINFO_NUM_INST_CODES,numCodes)
lindo.geterrormessage(pEnv,errorcode)
errorcode = lindo.pyLSgetInfo(pModel,lindo.LS_IINFO_NUM_INST_REAL_NUM,numVals)
lindo.geterrormessage(pEnv,errorcode)
...
for k,x in enumerate(panCode):    
    if panCode[k]==lindo.EP_PUSH_VAR:
        j = panCode[k+1]
        if paiTypeMap[j]>0:
            panCode[k] = lindo.EP_PUSH_NUM
            panCode[k+1]=padNumVal.size-1

#lindo.pyLSsetModelIntParameter(xModel,lindo.LS_IPARAM_INSTRUCT_SUBOUT,1)
...
errorcode = lindo.pyLSgetInfo(xModel,lindo.LS_IINFO_NUM_VARS,numVars)
lindo.geterrormessage(pEnv,errorcode)
errorcode = lindo.pyLSgetInfo(xModel,lindo.LS_IINFO_NUM_CONS,numCons)
lindo.geterrormessage(pEnv,errorcode)
errorcode = lindo.pyLSgetInfo(xModel,lindo.LS_IINFO_NUM_NLP_VARS,numNlpVars)
lindo.geterrormessage(pEnv,errorcode)
errorcode = lindo.pyLSgetInfo(xModel,lindo.LS_IINFO_NUM_INST_CODES,numCodes)
lindo.geterrormessage(pEnv,errorcode)
errorcode = lindo.pyLSgetInfo(xModel,lindo.LS_IINFO_NUM_INST_REAL_NUM,numVals)
lindo.geterrormessage(pEnv,errorcode)
numLpVars[0]=numVars[0]-numNlpVars[0]
print("numVars   : %d" %numVars[0])
print("numCons   : %d" %numCons[0])
...
errorcode = lindo.pyLSsetModelIntParameter(xModel,lindo.LS_IPARAM_SOLVER_IUSOL,1)
...
if 0>1:
    ...
    for k,x in enumerate(padPrimal): 
        if paiTypeMap[k]==0:
            print("-%.6f %8d" % (padPrimal[k],paiTypeMap[k]))    
if 0>1:
    errorcode = lindo.pyLSsetModelIntParameter(xModel,lindo.LS_IPARAM_IIS_METHOD,3)
    lindo.geterrormessage(pEnv,errorcode)
